...
def whatsp_auto(name,wp_message):
	...
	try:
		...
		texttoSpeech('you have encountered an error because of the two possibilies')
		texttoSpeech('1 Username may not be present in your contact list')
		texttoSpeech('2 May be present but not visible on the screen')
		print("No username Visible on screen please scroll down or no contact of that name!!")
		exit()
...
def check_flag(flag):
	if(flag.lower()=='y'):
		...
		whatsp_auto(name.strip(),wp_message)
	elif(flag.lower()=='n'):
		...
		texttoSpeech('Give a valid input')
		flag_space=input("Enter a value (y/n):  ") 
		check_flag(flag_space)
